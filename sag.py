...
for t in range(tmax):
    ...
    for k in range(rest):
        ...
        for w in range(nd):
            '''Geração do novo valor para o componente w do vetor solução'''
            for i in range(nd):
                if i == w:
                    x_new[i] = 1.1*xmax #forçando a entrada no whike
                    while x_new[i] < xmin or x_new[i] > xmax:
                        x_new[i] = x_old[i] + Tv*qgasdev(qv)
                else:
                    x_new[i] = x_old[i]
